...
import { QrCode, Camera, Check, Copy, AlertTriangle, AtSign } from "lucide-react"
...
import { KnsLookup, type KnsLookupResult } from "./KnsLookup"
...
export function AddContactWithKns({ handle }: { handle?: string }) {
  ...
  // KNS-specific state
  const [knsLookupResult, setKnsLookupResult] = useState<KnsLookupResult | null>(null)
  const [selectedContact, setSelectedContact] = useState<{ accountId: string; name?: string } | null>(null)
  ...
  // Handle KNS lookup result
  const handleKnsSelect = (result: KnsLookupResult) => {
    setKnsLookupResult(result)
    if (result.accountId) {
      setSelectedContact({
  ...
  // Connect to resolved contact
  async function connectToResolvedContact() {
    if (!selectedContact || !sessionProfile) return

    // Create a contact request to the resolved account
    const envelope = {
      type: "CONTACT_REQUEST",
      from: sessionProfile.sessionId,
      nonce: Date.now(),
      ts: Math.floor(Date.now() / 1000),
      payload: {
        to: selectedContact.accountId,
        fromProfileId: sessionProfile.sessionId,
        fromProfileHrl: sessionProfile.profileHrl,
        handle: sessionProfile.handle,
        knsName: selectedContact.name // Include KNS name if available
      },
      sig: "demo_signature"
    }
    ...
    // Add to signals store
    const signalEvent: SignalEvent = {
      id: `contact_request_kns_${envelope.nonce}`,
      class: "contact",
      topicType: "CONTACT",
      direction: "outbound",
      actors: { 
    ...
    const displayName = selectedContact.name || selectedContact.accountId
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        ...
        <Tabs defaultValue="kns" className="w-full">
          <TabsList className="grid grid-cols-3 mb-2">
            <TabsTrigger value="kns">
              <AtSign className="w-3 h-3 mr-1" /> @Name
            </TabsTrigger>
            <TabsTrigger value="mine">
              <QrCode className="w-3 h-3 mr-1" /> My QR
            </TabsTrigger>
          ...
          {/* KNS Name Lookup */}
          <TabsContent value="kns" className="space-y-3">
            <div className="space-y-3">
              <div>
                <Label className="text-sm font-medium">Find by Name</Label>
                <p className="text-xs text-slate-500 mb-2">
                  Enter a KNS name (@alice.hbar) or account ID (0.0.12345)
                </p>
              </div>
              
              <KnsLookup
                placeholder="@alice.hbar or 0.0.12345"
                onSelect={handleKnsSelect}
              ...
              {selectedContact && (
                <div className="space-y-2">
                  <Button 
                    onClick={connectToResolvedContact}
                    className="w-full bg-green-600 hover:bg-green-700 text-white"
                  >
                    <Check className="w-4 h-4 mr-2" />
                    Send Contact Request to {selectedContact.name || selectedContact.accountId}
                  </Button>
                </div>
              )}
            </div>
