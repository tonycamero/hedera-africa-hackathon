...
export const knsService = {
  ...
  /**
   * Reverse lookup: account ID to name
   */
  async lookupAccount(accountId: string): Promise<string | null> {
    if (!KNS_ENABLED) {
      throw new Error('KNS service is disabled');
    }
    ...
    // Check client cache first
    const cached = getCached<KNSReverseResult>(cacheKey);
    if (cached) {
      console.log(`[KNS Service] Client cache hit for reverse: ${normalizedAccountId}`);
      return cached.name;
    }
    
    try {
      console.log(`[KNS Service] Looking up account: ${normalizedAccountId}`);
      const result = await fetchWithTimeout<KNSReverseResult>(`/api/kns/reverse?accountId=${encodeURIComponent(normalizedAccountId)}`);
      
      // Cache the result
      setCached(cacheKey, result);
      
      console.log(`[KNS Service] Looked up ${normalizedAccountId} → ${result.name || 'NOT_FOUND'}`);
      return result.name;
      
    } catch (error) {
      console.error(`[KNS Service] Failed to lookup ${normalizedAccountId}:`, error);
      throw new Error(`Failed to lookup account: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  },
  ...
  async checkAvailability(name: string): Promise<boolean> {
    ...
    // Check client cache first
    const cached = getCached<KNSAvailableResult>(cacheKey);
    if (cached) {
      console.log(`[KNS Service] Client cache hit for availability: ${normalizedName}`);
      return cached.available;
    }
    
    try {
      console.log(`[KNS Service] Checking availability: ${normalizedName}`);
      const result = await fetchWithTimeout<KNSAvailableResult>(`/api/kns/available?name=${encodeURIComponent(normalizedName)}`);
      
      // Cache the result
      setCached(cacheKey, result);
      
      console.log(`[KNS Service] ${normalizedName} is ${result.available ? 'AVAILABLE' : 'TAKEN'}`);
      return result.available;
      
    } catch (error) {
      console.error(`[KNS Service] Failed to check availability for ${normalizedName}:`, error);
  ...
  async resolveIdentifier(input: string): Promise<{ 
    ...
    if (trimmed.match(/^0\.0\.\d+$/)) {
      // It's an account ID - try reverse lookup for display name
      try {
        const name = await this.lookupAccount(trimmed);
        return {
          type: 'account',
          value: trimmed,
        ...
        // Reverse lookup failed, just return the account ID
        return {
          type: 'account',
          value: trimmed
        };
